...
* **Making tests optional**: initially, e2e-tests were running automatically on each branch. We decided to run e2e-tests for feature branches only if the developer explicitly specified that. After three months of monitoring, we confirmed that the stability of the `master` branch was unaffected by this change. Unit tests, which we began prioritizing, proved sufficient for verifying changes before merging.
...
1. **Create 14 pull requests and get 14 successful builds**: Without the need for e2e-tests on each branch, it takes about **1 hour**.
    
2. **Merge the 14 pull requests into the** `master` **branch and get one “green” e2e-tests**: While we've optimized the test execution time, we still face occasional stability issues. As a result, this step may take up to **two e2e-test runs,** resulting in **30-60 minutes**.
